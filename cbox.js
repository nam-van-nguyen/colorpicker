...
// prop - A CSS property (such as 'background-color')
    ...
    function setPageX(e, x) {
        ...
        if (estyle) {
            if ("left" in estyle)
                estyle.left = x + "px";
            else if ("pixelLeft" in estyle)
                estyle.pixelLeft = x + "px";
    ...
    function setPageY(e, x) {
        ...
        if (estyle) {
            if ("top" in estyle)
                estyle.top = x + "px";
            else if ("pixelTop" in estyle)
                estyle.pixelTop = x + "px";
...
////////////////////////////////
    function setInputValue(input, value) {
        ...
        let changeListener = input["changeListener"];
        if ( changeListener ) {
            if ( oldVal !== value ) {
                //console.log(input);
                //console.log(changeListener);
                changeListener(value, oldVal);
...
// Returns null if the color can't be converted
    function colorToRgba(x) {
        ...
        function parsePercent(x) {
            var c;
            return ((c = parseFloat(x)) < 0 ? 0 : (c > 100 ? 100 : c)) * 255 / 100;
        }
        function parseAlpha(x) {
            var c;
            return ((c = parseFloat(x)) < 0 ? 0 : (c > 1 ? 1 : c)) * 255;
        }
        function parseByte(x) {
            var c;
            return ((c = parseInt(x, 10)) < 0 ? 0 : (c > 255 ? 255 : c));
        }
        function parseHue(x) {
...
// green, and blue (each from 0-255), with optional alpha (0-255)
    ...
    colorToRgba.namedColorsPattern = function () {
        ...
        for (var o in colorToRgba.namedColors) {
            ...
            if (typeof v === "string") {
                b[b.length] = o;
                if (o.indexOf("gray") >= 0)
                    b[b.length] = o.replace("gray", "grey");
            }
        }
        ...
        if (nodes.length === colors.length - 1) {
            ...
            for (i = 0; i < nodes.length; i++) {
                ...
                if (ht <= 0)
                    ...
                    setWidth(n, w);
                    setHeight(n, ht);
                    if (!applyCssGradient(n, [colors[i], colors[i + 1]]))
                        return false;
                }
            }
            return true;
    ...
    var applyCssGradient = function (o, colors) {
        ...
        if (bi.indexOf("linear-gradient(") !== 0)
        ...
        for (var j = 0; j < colorstrings.length; j++) {
            if (j === 0)
                gradientstring += ", from(";
            else if (j === colorstrings.length - 1)
                gradientstring += ", to(";
            else
                gradientstring += ", color-stop(" + (j * 1.0 / (colors.length - 1)) + ", ";
    ...
    var ColorSpace = subclass(Object, {
        ...
        setupdimensions: function () {
            if (!(typeof this.info.fromrgbcolor !== "undefined" && this.info.fromrgbcolor)) {
                if (!this.usealpha)
                    this.areas[1] = [0, 0, this.maxWidth, this.matrixHeight];
                else
                    this.areas[1] = [0, 0, this.matrixWidth +
                                Math.floor((this.maxWidth - this.matrixWidth) / 2), this.matrixHeight];
                // side bar
                this.areas[2] = [0, 0, 0, 0];
                // alpha bar
                if (this.usealpha)
                    this.areas[6] = [this.matrixWidth +
                ...
                // side bar
                if (this.usealpha)
                    this.areas[2] = [this.matrixWidth, 0,
                        Math.floor((this.maxWidth - this.matrixWidth) / 2),
                        this.matrixHeight];
                else
                    this.areas[2] = [this.matrixWidth, 0,
        ...
        confinetoarea: function (area, xy) {
            ...
            if (nxy[0] < dims[0])
                nxy[0] = dims[0];
            if (nxy[0] > dims[0] + dims[2] - 1)
                nxy[0] = dims[0] + dims[2] - 1;
            if (nxy[1] < dims[1])
                nxy[1] = dims[1];
            if (nxy[1] > dims[1] + dims[3] - 1)
                nxy[1] = dims[1] + dims[3] - 1;
            return nxy;
        },
        ...
        colortopos: function (current) {
            ...
            for (var i = 0; i < this.info.maxes.length; i++) {
                ...
                if (this.info.reversed[this.info.indexes[i]])
                    v = this.info.maxes[this.info.indexes[i]] - v;
                v /= this.info.maxes[this.info.indexes[i]];
                if (i === 0)
                    v = dims[0] + v * (dims[2] - 1);// matrix X
                if (i === 1)
                    v = dims[1] + v * (dims[3] - 1);// matrix Y
                if (i === 2)
                    v = dimsside[1] + v * (dimsside[3] - 1);// side Y
                ret[i] = v;
            }
        ...
        changecolor: function (x, y, current) {
            ...
            if (x >= area[0] && x < (area[0] + area[2]) &&
                ...
                if (info.reversed[ci0])
                    h = info.maxes[ci0] - h;
                if (info.reversed[ci1])
                    s = info.maxes[ci1] - s;
                ret[ci0] = h;
                ret[ci1] = s;
                return ret;
            ...
            if (x >= area[0] && x < (area[0] + area[2]) &&
                ...
                if (info.reversed[ci2])
                    lum = info.maxes[ci2] - lum;
                ret[ci2] = lum;
                return ret;
            }
...
////////////////
    ...
    var namedColorsDatalist2 = function () {
        if ((_namedColorsDatalist2 !== null && typeof _namedColorsDatalist2 !== "undefined"))
...
// is deprecated
    ...
    var useNativeColorPicker = function (thisInput, usealpha) {
        ...
        if (!usealpha && supportsColorInput() && (thisInput.type === "text" || thisInput.type === "color")) {
            ...
            setInputValue(thisInput, normalizedRgb);
            ...
            if (thisInput.nextSibling)
                thisInput.parentNode.insertBefore(infolink, thisInput.nextSibling);
            else
                thisInput.parentNode.appendChild(infolink);
            var thisInputBlur = function () {
                if (thisInput.type === "text") {
                    ...
                    thisInput.setAttribute("list", datalistid);
                    thisInput.title = "";
                    setInputValue(thisInput, oldvalue);
            ...
            addListener(infolink, (window.opera ? "mouseup" : "click"), function () {
                ...
                setInputValue(thisInput, rgbToColorDisplay(colorToRgb(currentValue)));
    ...
    var supportsPattern = function () {
        if ((_supportsPattern !== null && typeof _supportsPattern !== "undefined"))
    ...
    var setPatternAndTitle = function (thisInput, usealpha) {
        ...
        if (thisInput.tagName.toLowerCase() === "input" && thisInput.type === "text") {
            ...
            thisInput.setAttribute("list", datalistid);
            if (!namedPattern)
                namedPattern = colorToRgba.namedColorsPattern();
...
////////////////
    var MyColorPicker = subclass(Object, {
        initialize: function (info, parent, startingvalue, usealpha) {
            ...
            if (this.usealpha)
                this.swatchbg.style.display = "none";
            ...
            if (this.ishueslider) {
                ...
                if (!this.updateHueSlider(this.hueslider, this.current))
                    this.ishueslider = false;
            }
            if (this.isoriginal) {
            ...
            if (!this.usealpha)
                this.cursors[2].style.display = "none";
            this.currentArea = 0;
            this.adjustPos();
        ...
        documentKeyDown: function (e) {
            ...
            if (key === 9) { // tab
                ...
                if ("focus" in this.resetlink) {
                    if (this.focusedCursor === choices - 1)
        ...
        updatedivs: function (area) {
            ...
            if (this.ishueslider && area !== 2) {
                ...
                if (this.isoriginal && !this.usealpha)
                    justswatch = true;
            }
            if (this.isoriginal) {
                var areadims = this.colorspace.areadimensions(1);
    ...
    rootobj.setRgba = function (thisInput, cc) {
        ...
        if (attr === "1")
            setInputValue(thisInput, rgbToColorRgba(cc));
            //thisInput.value = rgbToColorRgba(cc);
        else if (attr === "2")
            setInputValue(thisInput, rgbToColorArgb(cc));
            //thisInput.value = rgbToColorArgb(cc);
        else
            setInputValue(thisInput, rgbToColorDisplay(cc));
            //thisInput.value = rgbToColorDisplay(cc);
    };
    rootobj.normalizeRgb = function (thisInput) {
        var attr = thisInput.getAttribute("rgbahex");
        if (attr === "1")
            return rgbaToColorRgba(colorRgbaToRgba(thisInput.value));
        else
        if (attr === "2")
            return rgbaToColorArgb(colorArgbToRgba(thisInput.value));
        else
    ...
    rootobj.getRgbaOrNull = function (thisInput) {
        ...
        if (attr === "1")
            ...
            return colorArgbToRgba(thisInput.value);
        else if (thisInput.getAttribute("usealpha") === "1")
            return colorToRgba(thisInput.value);
        else
            return colorToRgb(thisInput.value);
    };
    ...
    var supportsColorInput = function () {
        if ((_supportsColorInput !== null && typeof _supportsColorInput !== "undefined"))
    ...
    function onNewInputClickFunction(newInput, thisInput, extra) {
        return function () {
            ...
            if (!extra.flat) {
                ...
                if (newInput)
                    newInput.parentNode.insertBefore(o, newInput);
                else
